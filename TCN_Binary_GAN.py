...
def build_generator(input_shape):
    X = TCN(input_shape=input_shape)
    X = Dense(48, activation='tanh')(X)  # 48 outputs (x,y,z coordinate for every keypoint)
    model = Model(inputs=input_shape, outputs=X, name='Generator')
    return model
...
if __name__ == '__main__':
    ...
    gen_input_shape = (12, 1)  # 12 frames processed with 1 dimension
    disc_input_shape = (16, 2)  # 16 keypoints with x,y coordinates
    batch_size = 256
    ...
    discriminator = build_discriminator(disc_input_shape)
