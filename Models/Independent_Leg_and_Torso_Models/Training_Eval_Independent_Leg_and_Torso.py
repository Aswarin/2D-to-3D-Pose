...
loss_type = 'MSE'
model_number = '1'
generator_weight_files = 'leg_and_torso_weights/' + loss_type + '_' + model_number + '/' # Load in the generator weights from a h5 file
...
for i in range(len(legs_weights)):
    ...
    for j in range(len(predicted_3d_poses)):
        ...
        error = (np.sqrt(np.sum((ground_truth_val_data[j] - Z) ** 2, axis=1)))
