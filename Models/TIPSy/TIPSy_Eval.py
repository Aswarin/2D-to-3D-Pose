import os
import numpy as np
import json
from simple_student_model import build_student_generator
import pandas as pd
import tensorflow as tf

"""
This file is used to perform procrustes alignment to the predicted output. 
What this does is scale and rotate the predicted 3D pose from the generator the minimise the distance 
between this pose and the ground truth 3D pose. No manipulation is done this is purely scaling and rotation as 
the predicted 3D pose is no to scale with the ground truth 3D pose and therefore a direct comparison is not 
feasible
"""

def procrustes(X, Y, scaling=True, reflection='best'):
    """
    A port of MATLAB's `procrustes` function to Numpy.

    Procrustes analysis determines a linear transformation (translation,
    reflection, orthogonal rotation and scaling) of the points in Y to best
    conform them to the points in matrix X, using the sum of squared errors
    as the goodness of fit criterion.

        d, Z, [tform] = procrustes(X, Y)

    Inputs:
    ------------
    X, Y
        matrices of target and input coordinates. they must have equal
        numbers of  points (rows), but Y may have fewer dimensions
        (columns) than X.

    scaling
        if False, the scaling component of the transformation is forced
        to 1

    reflection
        if 'best' (default), the transformation solution may or may not
        include a reflection component, depending on which fits the data
        best. setting reflection to True or False forces a solution with
        reflection or no reflection respectively.

    Outputs
    ------------
    d
        the residual sum of squared errors, normalized according to a
        measure of the scale of X, ((X - X.mean(0))**2).sum()

    Z
        the matrix of transformed Y-values

    tform
        a dict specifying the rotation, translation and scaling that
        maps X --> Y

    """

    n, m = X.shape
    ny, my = Y.shape
    muX = X.mean(0)
    muY = Y.mean(0)

    X0 = X - muX
    Y0 = Y - muY

    ssX = (X0 ** 2.).sum()
    ssY = (Y0 ** 2.).sum()

    # centred Frobenius norm
    normX = np.sqrt(ssX)
    normY = np.sqrt(ssY)

    # scale to equal (unit) norm
    X0 /= normX
    Y0 /= normY

    if my < m:
        Y0 = np.concatenate((Y0, np.zeros(n, m - my)), 0)

    # optimum rotation matrix of Y
    A = np.dot(X0.T, Y0)
    U, s, Vt = np.linalg.svd(A, full_matrices=False)
    V = Vt.T
    T = np.dot(V, U.T)

    if reflection != 'best':

        # does the current solution use a reflection?
        have_reflection = np.linalg.det(T) < 0

        # if that's not what was specified, force another reflection
        if reflection != have_reflection:
            V[:, -1] *= -1
            s[-1] *= -1
            T = np.dot(V, U.T)

    traceTA = s.sum()

    if scaling:

        # optimum scaling of Y
        b = traceTA * normX / normY

        # standarised distance between X and b*Y*T + c
        d = 1 - traceTA ** 2

        # transformed coords
        Z = normX * traceTA * np.dot(Y0, T) + muX

    else:
        b = 1
        d = 1 + ssY / ssX - 2 * traceTA * normY / normX
        Z = normY * np.dot(Y0, T) + muX

    # transformation matrix
    if my < m:
        T = T[:my, :]
    c = muX - b * np.dot(muY, T)

    # transformation values
    tform = {'rotation': T, 'scale': b, 'translation': c}

    return d, Z, tform


weight_list = []
all_errors = []
input_size = (16, 2)  # The input to the generator and discriminator [16 (x,y) keypoints]
student = build_student_generator(input_size)
student_weight_location = 'student_weights/'  # Load in the generator weights from a h5 file
test_file = '../full_final_testing_data.json'

with open(test_file) as f:
    test_data = np.array(json.load(f))

ground_truth_val_data = np.array([t['3d_pose'][1:] for t in test_data])
action_list = [t['action'] for t in test_data]
input_val_data = tf.convert_to_tensor([t['norm_2d_pose'] for t in test_data])


best_error = 99999
best_weight = None
# Build generator and load in its weights
for weight_file in os.listdir(student_weight_location):
    weight_list.append(weight_file)

weight_list.sort()

for weight_file in weight_list:
    student.load_weights(student_weight_location + weight_file)
    predictions = student.predict(input_val_data, batch_size=8192)
    predictions = np.reshape(predictions, (len(predictions), 16, 1))
    predicted_3d_poses = np.concatenate((input_val_data, predictions), axis=2)
    error = []
    for j in range(len(predicted_3d_poses)):
        # Perform procrustes_alignment
        d, Z, tform = procrustes(ground_truth_val_data[j], predicted_3d_poses[j])
        error.append(np.mean(np.sqrt(np.sum((ground_truth_val_data[j] - Z) ** 2, axis=1))))

    if np.mean(error) < best_error:
        best_error = np.mean(error)
        best_weight = weight_file
        best_predictions = predicted_3d_poses
    all_errors.append(np.mean(error))

    print('The average error for weights ' + weight_file + ' the validation set is %.3f MM' % np.mean(error))
    print('The best error so far is ' + str(best_error) + ' using weights ' + str(best_weight))
    print()

print('The best error was ' + str(best_error) + ' using weights ' + str(best_weight))
all_errors = pd.DataFrame(all_errors)
all_errors.to_csv('solo_student_errors.csv', index=False)
with open('student_predictions.json', 'w') as outfile:
    json.dump(best_predictions.tolist(), outfile)

