...
import numpy as np
import json
from MSE_Split_Combine_5_parts_model import build_body_generator
import pandas as pd
...
five_way_split_combine_model = build_body_generator(input_size)
loss_type = 'MSE'
model_number = '1'
five_way_split_combine_model_weight_location = '5_way_split_combine_weights/' + loss_type + '_' + model_number + '/' # Load in the generator weights from a h5 file
...
weight_list = []
for weight_file in os.listdir(five_way_split_combine_model_weight_location):
    weight_list.append(weight_file)

weight_list.sort()
all_errors = []
for weight_file in weight_list:
    five_way_split_combine_model.load_weights(five_way_split_combine_model_weight_location + weight_file)
    predictions = five_way_split_combine_model.predict(input_val_data, batch_size=8192)
...
all_errors.to_csv('5_way_split_combine_model_errors' + loss_type + '_' + model_number + '.csv', index=False)
