...
def independent_limb_generator(torso_keypoints):
...
def build_body_generator(input_shape):
    ...
    right_leg_output = independent_limb_generator(body_input[:, 0:3, :])
    left_leg_output = independent_limb_generator(body_input[:, 3:6, :])
    torso_output = independent_torso_generator(body_input[:, 6:10, :])
    left_arm_output = independent_limb_generator(body_input[:, 10:13, :])
    right_arm_output = independent_limb_generator(body_input[:, 13:16, :])
    X = Concatenate()([right_leg_output, left_leg_output, torso_output, left_arm_output, right_arm_output])
    X = residual_block(X, 0.5)
...
class TIPSy(Model):
    ...
    def train_step(self, pose_batch):
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            cross_entropy_loss = self.loss_func(misleading_labels, predictions)
            """Discriminator Loss"""

            generator_loss = (cross_entropy_loss * 0.15) + (comparison_2d_loss * 10) + (consistency_loss * 3)
...
if __name__ == '__main__':
    ...
    gen_filepath = '5_way_split_combine/GAN_1/'
